...
#
# A triplet here is defined as a sequence of three letters such that each letter occurs somewhere before the next
...
def pop_val(a,b,alist):  # a need to be before b ; b should exists, otherwise, insert a if not in array
    indexA = alist.index(a)
    indexB = alist.index(b)
    if indexB <= indexA:
        alist.remove(a)
        alist.insert(indexB, a)

def recoverSecret(triplets):
    answer = ''
    listweigth = []
    for x,y,z in triplets:
        if x not in listweigth:
            listweigth.append(x)
        if y not in listweigth:
            listweigth.append(y)
        if z not in listweigth:
            listweigth.append(z)
        pop_val(y, z, listweigth)
        pop_val(x, z, listweigth)
        pop_val(x, y, listweigth)
    for x, y, z in triplets:
        pop_val(y, z, listweigth)
        pop_val(x, z, listweigth)
        pop_val(x, y, listweigth)
